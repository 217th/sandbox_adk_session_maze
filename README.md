...
Учебный агент, демонстрирующий работу с состоянием (`session.state`), flash-данными и журналом событий (`session.events`) в Google ADK. Пользователь перемещается по сетке 3×3, избегает стен и фиксирует каждую попытку хода. Проект показывает, как отделять:
